...
async def upload(ev, bot, jdb, message_edited=None):
    ...
    index = -1
    cloudtype = user_data['cloudtype']
    uptype = user_data['uploadtype']
    splitsize = -1
    buttons = None
            ...
            if loged:
                ...
                for item in files:
                    ...
                    if uptype == "draft":
                        ...
                        itemid, data = await client.upload_file_draft(
                            item, progress_upload, (bot, message)
                        )
                    ...
                    if uptype == "evidencia":
                        ...
                        if len(files) > 1:
                            text += "📚 " + str(len(files)) + " Partes\n"
                        await message.edit(text, buttons=buttonsadd)
                        itemid, data = await client.upload_file(
                            item, evidence, itemid, progress_upload, (bot, message)
                        )
                        text = "💚 Subiendo con Éxito 💚\n\n"
                        text += "👨🏻‍💻 " + filefullname + "\n"
                        text += "📦Tamaño Total: " + sizeof_fmt(filefullsize) + " \n"
                    if uptype == "blog":
                        ...
                        if len(files) > 1:
                            text += "📚 " + str(len(files)) + " Partes\n"
                        await message.edit(text, buttons=buttonsadd)
                        itemid, data = await client.upload_file_blog(
                            item, blog, itemid, progress_upload, (bot, message)
                        )
                        text = "💚 Subiendo con Éxito 💚\n\n"
                        text += "👨🏻‍💻 " + filefullname + "\n"
                        text += "📦Tamaño Total: " + sizeof_fmt(filefullsize) + " \n"
