...
class Downloader(object):
    ...
    async def _process_mega_download(self,megaurl,progressfunc,args):
        megadl = Mega({'verbose': True})
        megadl.login()
        info = megadl.get_public_url_info(megaurl)
        self.filename = info['name']
        destpath = self.destpath.replace('/','')
        await megadl.download_url(megaurl,dest_path=destpath,dest_filename=self.filename,progressfunc=progressfunc,args=args)
